...
@Composable
fun AmityUserFollowerListComponent(
    ...
    val loadState by viewModel.followerListState.collectAsState()
    ...
    LazyColumn(modifier.fillMaxSize()) {
        ...
        ).let(viewModel::setFollowerListState)
        ...
        when (loadState) {
            ...
            AmityUserRelationshipPageViewModel.FollowListState.SUCCESS -> {
                items(
                    ...
                    key = { index -> lazyPagingItems[index]?.getSourceUser()?.getUserId() ?: index }
                ) { index ->
                    val follower = lazyPagingItems[index]?.getSourceUser() ?: return@items

                    AmityUserListItem(
                        user = follower,
                        showRightMenu = follower.getUserId() != AmityCoreClient.getUserId(),
                        onUserClick = {
