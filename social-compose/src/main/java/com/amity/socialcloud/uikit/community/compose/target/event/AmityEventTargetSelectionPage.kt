...
@Composable
fun AmityEventTargetSelectionPage(
    modifier: Modifier = Modifier,
) {
    val context = LocalContext.current
    val behavior by lazy {
        AmitySocialBehaviorHelper.eventTargetSelectionPageBehavior
    }
    ...
    AmityBasePage(
        ...
        Column(
            ...
            AmityTargetSelectionMyCommunitiesView(
                modifier = modifier,
                contentType = AmityTargetContentType.Event
            ) {
                behavior.goToEventSetupPage(
                    context = context,
                    launcher = launcher,
                    targetId = it.getCommunityId(),
                    targetType = AmityEventTargetType.COMMUNITY,
                    mode = AmityEventSetupPageMode.Create(communityId = it.getCommunityId())
                )
            }
        }
    }
...
enum class AmityEventTargetType {
    USER,
    COMMUNITY
}
